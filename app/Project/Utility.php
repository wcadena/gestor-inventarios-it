    ...
    public static function getWorkspaceBySlug($slug)
    {
        ...
        if ($objUser && $objUser->currant_workspace) {
            if ($objUser->getGuard() == 'client') {
                $rs = Workspace::select(['workspaces.id', 'workspaces.lang', 'workspaces.name', 'workspaces.slug', 'workspaces.created_by'])->join('client_workspaces', 'workspaces.id', '=', 'client_workspaces.workspace_id')->where('workspaces.id', '=', $objUser->currant_workspace)->where('client_id', '=', $objUser->id)->first();
            } else {
        ...
        if ($objUser && !empty($slug)) {
            if ($objUser->getGuard() == 'client') {
                $rs = Workspace::select(['workspaces.id', 'workspaces.lang', 'workspaces.name', 'workspaces.slug', 'workspaces.created_by'])->join('client_workspaces', 'workspaces.id', '=', 'client_workspaces.workspace_id')->where('slug', '=', $slug)->where('client_id', '=', $objUser->id)->first();
            } else {
        ...
        if ($objUser) {
            if ($objUser->getGuard() == 'client') {
                $rs = Workspace::select(['workspaces.id', 'workspaces.lang', 'workspaces.name', 'workspaces.slug', 'workspaces.created_by'])->join('client_workspaces', 'workspaces.id', '=', 'client_workspaces.workspace_id')->where('client_id', '=', $objUser->id)->orderBy('workspaces.id', 'desc')->limit(1)->first();
            } else {
