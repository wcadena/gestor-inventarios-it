...
@section('content')
    @if($project && $currantWorkspace && $task)
        ...
        <form class="pl-3 pr-3" method="post" action="@auth('web'){{ route('project.tasks.update',[$currantWorkspace->slug,$project->id,$task->id]) }}@elseauth{{ route('client.tasks.update',[$currantWorkspace->slug,$project->id,$task->id]) }}@endauth">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-md-8">
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" value="{{$task->due_date}}" class="form-control form-control-light date" id="due_date" data-provide="datepicker" data-date-format="yy-mm-dd" name="due_date" required autocomplete="off">
                    </div>
                </div>
            </div>
        ...
        <script>
          $(document).on('change',"select[name=project_id]",function () {
            $.get('@auth('web'){{route('project.home')}}@elseauth{{route('client.home')}}@endauth'+'/userProjectJson/'+$(this).val(),function (data) {
            ...
            $.get('@auth('web'){{route('project.home')}}@elseauth{{route('client.home')}}@endauth'+'/projectMilestoneJson/'+$(this).val(),function (data) {
