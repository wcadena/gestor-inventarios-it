@if($timesheet && $currantWorkspace && $project)
    <form class="pl-3 pr-3" method="post" action="{{ route('projects.timesheet.update',[$currantWorkspace->slug,$timesheet->id]) }}">
        @csrf
        @method('put')
        <div class="row">
            <div class="col-md-12">
                    ...
                    <select class="form-control form-control-light" name="task_id" id="task_id" required>
                        ...
                        @if(!empty($project->tasks()))
                            @foreach($project->tasks() as $task)
                                <option value="{{ $task->id }}" {{ ($task->id == $timesheet->task_id) ? 'selected' : '' }}>{{ $task->title }}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
