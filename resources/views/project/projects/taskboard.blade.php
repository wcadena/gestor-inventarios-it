                                        ...
                                        <div class="card-body p-3">
                                            <div class="dropdown float-right">
                                                ...
                                                @elseif(isset($permisions))
                                                    ...
                                                    @endif
                                                    @if(in_array('delete task',$permisions))
                                                        <a href="#" class="dropdown-item" onclick="(confirm('Are you sure ?')?document.getElementById('delete-form-{{$taskDetail->id}}').submit(): '');">
                                                            <i class="mdi mdi-delete mr-1"></i>{{__('Delete')}}
                                                        </a>
                                                        <form id="delete-form-{{$taskDetail->id}}" action="{{ route('client.tasks.destroy',[$currantWorkspace->slug,$taskDetail->project_id,$taskDetail->id]) }}" method="POST" style="display: none;">
                                                            @csrf
                                                            @method('DELETE')
                                                        </form>
                                                    @endif
...
@push('scripts')
    ...
    <script>
        !function (a) {
            ...
            "use strict";
            @auth('client')
                @if(isset($permisions) && in_array('move task',$permisions))
                    a.Dragula.init()
                @endif
            @elseauth
            a.Dragula.init()
