<aside id="sidebar-wrapper">
  ...
  <div class="sidebar-brand sidebar-brand-sm">
    <a href="{{ route('project.home') }}"><img src="{{asset(Storage::url('logo/logo.png'))}}" alt="{{ env('APP_NAME') }}" height="25"></a>
  </div>
  <ul class="sidebar-menu">
      ...
      @if(isset($currantWorkspace) && $currantWorkspace)
          <li class="{{ (Request::route()->getName() == 'projects.index') ? ' active' : '' }}">
              <a class="nav-link" href="@auth('web'){{ route('projects.index',$currantWorkspace->slug) }}@elseauth{{ route('project.client.projects.index',$currantWorkspace->slug) }}@endauth">
          ...
          @auth('web')
              <li class="{{ (Request::route()->getName() == 'users.index') ? ' active' : '' }}">
                  <a href="{{ route('project.users.index',$currantWorkspace->slug) }}">
              ...
              @if($currantWorkspace->creater->id == Auth::user()->id)
                  <li class="{{ (Request::route()->getName() == 'clients.index') ? ' active' : '' }}">
                      <a href="{{ route('project.clients.index',$currantWorkspace->slug) }}">
              ...
              <li class="{{ (Request::route()->getName() == 'calender.index') ? ' active' : '' }}">
                  <a href="{{route('project.calender.index',$currantWorkspace->slug)}}">
                      <i class="dripicons-calendar"></i>
                      <span> {{ __('Calender') }} </span>
                  </a>
              </li>
              <li class="{{ (Request::route()->getName() == 'notes.index') ? ' active' : '' }}">
                  <a href="{{route('project.notes.index',$currantWorkspace->slug)}}">
